...
limitations under the License.
  ...
  memcached proxy-protocol parser fuzz harness (from OSS-Fuzz projects/memcached).
  Drives try_read_command_proxy() over attacker-controlled bytes: each call parses a
  memcached command line (text + meta protocol) via proto_parser.c's process_request()
  on the always-on proxy connection set up in LLVMFuzzerInitialize().
*/
...
/* Bake detect_leaks=0 into the binary via the weak ASan default-options hook.
 * Mayhem owns the runtime ASAN_OPTIONS (abort_on_error=1, symbolize=0, ...) and a
 * Mayhemfile env value would REPLACE that whole set, so we must NOT set ASAN_OPTIONS
 * in the Mayhemfile. This weak symbol is additive and is not overridden by Mayhem's
 * runtime options. The proxy connection holds persistent global state (lua VM, conn,
 * thread caches) across iterations, so libFuzzer's leak detector reports benign
 * "leaks" of that intentionally-retained state; disable just leak detection. */
